...
class ExecutorInterface:
    ...
    def _ros2_execute(self, action: str, target: Optional[str]) -> bool:
        if action == "move_base":
            # Navigation is handled by PlanPush → Nav2 NavigateToPose.
            # If move_base arrives here it means plan_push was bypassed
            # (e.g. no world model available). Fall back to a short
            # forward nudge so the robot isn't completely stuck.
            logger.warning(
                "ExecutorInterface: move_base reached executor — "
                "navigation should go through PlanPush. Nudging forward."
            )
            self._publish_twist(linear=0.15, angular=0.0)
            time.sleep(1.0)
            self._publish_twist(0.0, 0.0)
            return True
        ...
        if action == "stop_for_human":
            # Stop all movement immediately — person detected
            self._publish_twist(0.0, 0.0)
            self._publish_string("/audio/speak", "Person detected. Stopping.")
            logger.info("ExecutorInterface: STOPPED for human")
            time.sleep(2.0)
            return True

        if action == "reroute_right":
            # Turn right 90 degrees then move forward to avoid obstacle
            self._publish_twist(0.0, -0.5)   # rotate right
            time.sleep(1.5)
            self._publish_twist(0.3, 0.0)    # move forward
            time.sleep(1.0)
            self._publish_twist(0.0, 0.5)    # rotate back left
            time.sleep(1.5)
            self._publish_twist(0.0, 0.0)
            logger.info("ExecutorInterface: rerouted RIGHT around obstacle")
            return True

        if action == "reroute_left":
            # Turn left 90 degrees then move forward to avoid obstacle
            self._publish_twist(0.0, 0.5)    # rotate left
            time.sleep(1.5)
            self._publish_twist(0.3, 0.0)    # move forward
            time.sleep(1.0)
            self._publish_twist(0.0, -0.5)   # rotate back right
            time.sleep(1.5)
            self._publish_twist(0.0, 0.0)
            logger.info("ExecutorInterface: rerouted LEFT around obstacle")
            return True
